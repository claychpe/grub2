...
grub_efi_service_binding (grub_efi_handle_t dev, grub_efi_guid_t *service_binding_guid)
{
  ...
  if (!service)
    {
      grub_error (GRUB_ERR_IO, N_("couldn't open http service binding protocol"));
      return NULL;
    }
  ...
  for (dev = net_devices; dev; dev = dev->next)
    {
      ...
      status = efi_call_4 (dev->ip4_config->set_data, dev->ip4_config,
		    ...
		    sizeof (ip4_policy), &ip4_policy);

      if (status != GRUB_EFI_SUCCESS)
	grub_printf ("Set IP4 STATIC POLICY FAILED, reason: %d\n", (int)status);
      else
	grub_printf ("Set IP4 STATIC POLICY SUCCEEDED\n");


      status = efi_call_4 (dev->ip6_config->set_data, dev->ip6_config,
		    GRUB_EFI_IP6_CONFIG_DATA_TYPE_POLICY,
		    sizeof (ip6_policy), &ip6_policy);

      if (status != GRUB_EFI_SUCCESS)
	grub_printf ("Set IP6 STATIC POLICY FAILED, reason %d\n", (int)status);
      else
	grub_printf ("Set IP6 STATIC POLICY SUCCEEDED\n");
    }
}
...
grub_efi_net_find_cards (void)
{
  ...
  for (id = 0, handle = handles; num_handles--; handle++, id++)
	...
	continue;
      ...
      http = grub_efi_open_protocol (http_handle, &http_guid,
			      GRUB_EFI_OPEN_PROTOCOL_GET_PROTOCOL);
	...
	continue;
      ...
      dhcp4 = grub_efi_open_protocol (dhcp4_handle, &dhcp4_guid,
			      GRUB_EFI_OPEN_PROTOCOL_GET_PROTOCOL);
